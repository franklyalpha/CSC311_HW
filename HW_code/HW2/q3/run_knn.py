from HW2.q3.l2_distance import l2_distance
...
def run_knn():
    ...
    for k in k_val:
        ...
        for i in range(len(prediction)):
            if prediction[i] == valid_targets[i]:
                match += 1
        print("k = {}, accuracy = {}".format(k, match / len(prediction)))
        accuracy.append(match / len(prediction))
    plt.bar(k_val, accuracy)
    plt.show()
    # below code is for test_set accuracy
    prediction = knn(5, train_inputs, train_targets, test_inputs)
    match = 0
    for i in range(len(prediction)):
        if prediction[i] == valid_targets[i]:
            match += 1
    print("k = {}, accuracy = {}".format(5, match / len(prediction)))
