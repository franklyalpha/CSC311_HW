...
def train_mle_estimator(train_images, train_labels):
    ...
    # theta mle: first find all data with label, then consider how many data has feature j;
    # that would be one row of theta matrix;
    theta_mle = []
    pi_labels = []
    for digit_type in range(10):
        boolean_array = []
        # N, M = train_images.shape
        for labels in train_labels:
            if labels[digit_type] == 1:
                boolean_array.append(True)
            else:
                boolean_array.append(False)

        # this is for analyzing pi_mle
        pi_labels.append(boolean_array.count(True))

        data_with_type = train_images[boolean_array]

        # now begin analysis of counting each feature
        data_array = np.asarray(data_with_type)
        prob_true = np.sum(data_array, axis=0) / data_array.shape[0]
        theta_mle.append(prob_true)
    theta_mle = np.asarray(theta_mle).T

    # now work on pi_mle
    # deduction suggests the label is total traning sample with this label divided by total
    # num of labels.
    pi_mle = np.asarray(pi_labels) / np.asmatrix(train_images).shape[0]
...
def train_map_estimator(train_images, train_labels):
    ...
    for digit_type in range(10):
        ...
        data_with_type = train_images[boolean_array]
        data_array = np.asarray(data_with_type)
        prob_true = (np.sum(data_array, axis=0) + 2) / (data_array.shape[0] + 4)
        theta_map.append(prob_true)
    theta_map = np.asarray(theta_map).T

    # now work on pi_mle
    # deduction suggests the label is total traning sample with this label divided by total
    # num of labels.
    pi_map = np.asarray(pi_labels) / np.asmatrix(train_images).shape[0]
...
def log_likelihood(images, theta, pi):
    ...
    log_l_theta = np.log(1 - theta)
    l_image = 1 - np.asarray(images)
    log_like = np.dot(images, log_theta) + np.dot(l_image, log_l_theta) + log_pi
    log_like -= np.log(np.dot(np.exp(log_like), pi).reshape(log_like.shape[0], 1))
    return log_like
...
def predict(log_like):
    ...
    # YOU NEED TO WRITE THIS PART
    # realizing that log function is increasing -> higher value indicates higher probability
    predict_index = np.argmax(log_like, axis=1)
...
def accuracy(log_like, labels):
    ...
    acc = np.count_nonzero(prediction == label_index) / log_like.shape[0]
    return acc
